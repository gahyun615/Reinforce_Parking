"""
Double DQN Agent (Value-based RL variant)
...
class DoubleDQNAgent:
    ...
    def select_action(self, state: np.ndarray, training: bool = True) -> int:
        if training:
            self.epsilon = max(
                self.epsilon_end,
                self.epsilon_end + (1.0 - self.epsilon_end) * (1 - self._step / self.epsilon_decay),
            )
    ...
    def update(self) -> float | None:
        ...
        with torch.no_grad():
            next_actions_online = self.q_net(next_states_t).argmax(dim=1, keepdim=True)
            next_q_target = self.target_net(next_states_t).gather(1, next_actions_online).squeeze(1)
            target = rewards_t + self.gamma * next_q_target * (1 - dones_t)
